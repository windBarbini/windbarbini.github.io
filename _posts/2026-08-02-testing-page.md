---
layout: post
title:  "Test"
date:   2026-08-02-18T00:00:00-00:00
author: Emily Barbini
categories: art
tags: Cross-Stitch
---


<h2>Page Testing</h2>
Very very rough WIP!

<hr>

Select from the preferences below to generate an editable cross-stitch pattern from an observed weather balloon hodograph!
<i>(Please note that this generator uses soundings from the University of Wyoming's radiosonde archive, so some soundings might be missing.)</i>

<hr>

<h1><b>Step 1.</b> Select your Preferences</h1>
<label for="times">Select a Time (in UTC/Zulu):</label>
	<select id="times" name="time">
		<option value="00" name="00"</option>
		<option value="01" name="01"</option>
		<option value="02" name="02"</option>
		<option value="03" name="03"</option>
		<option value="04" name="04"</option>
		<option value="05" name="05"</option>
		<option value="06" name="06"</option>
		<option value="07" name="07"</option>
		<option value="08" name="08"</option>
		<option value="09" name="09"</option>
		<option value="10" name="10"</option>
		<option value="11" name="11"</option>
		<option value="12" name="12"</option>
		<option value="13" name="13"</option>
		<option value="14" name="14"</option>
		<option value="15" name="15"</option>
		<option value="16" name="16"</option>
		<option value="17" name="17"</option>
		<option value="18" name="18"</option>
		<option value="19" name="19"</option>
		<option value="20" name="20"</option>
		<option value="21" name="21"</option>
		<option value="22" name="22"</option>
		<option value="23" name="23"</option>
	</select>

<hr>

<h1><b>Step 2.</b> Generate the Pattern</h1>
"Generate Pattern" button goes here
Pattern and reference(s) should appear below:


<hr>

<h1><b>Step 3.</b> Download the pattern, edit as needed, and sew!</h1>

<hr>

<h1><b>Pixel Art-Pattern Translations</b></h1>

<hr>

<h1><b>Tutorial</b></h1>

<hr>

<h1><b>Pattern Gallery</b></h1>

<hr>

<h1><b>Credits</b></h1>